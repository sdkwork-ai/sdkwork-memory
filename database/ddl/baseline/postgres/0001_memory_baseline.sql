...
CREATE TABLE IF NOT EXISTS ai_record (
  ...
  valid_to TEXT,
  expires_at TEXT,
  status VARCHAR(32) NOT NULL CHECK (status IN ('active','deleted','superseded')),
  sensitivity_level VARCHAR(32) NOT NULL,
  metadata_json TEXT,
  tags_json TEXT,
...
CREATE TABLE IF NOT EXISTS ai_candidate (
  ...
  decision_state VARCHAR(32) NOT NULL CHECK (decision_state IN ('pending','approved','rejected')),
...
CREATE TABLE IF NOT EXISTS ai_eval_run (
  ...
  state VARCHAR(32) NOT NULL CHECK (state IN ('accepted','queued','running','succeeded','failed','dead')),
  metrics_json TEXT,
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS ai_outbox_event (
  ...
  payload_json TEXT NOT NULL,
  publish_state VARCHAR(32) NOT NULL CHECK (publish_state IN ('pending','processing','published','failed')),
  published_at TEXT,
