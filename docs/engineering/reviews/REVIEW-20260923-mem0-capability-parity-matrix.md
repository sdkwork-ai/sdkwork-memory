...
时间锚点与 few-shot 是提示词资源增强。


**批次 18 补记（2026-09-25，`fabbed0`）—— few-shot 示例内联（部分 #7 关闭）**：
「12 个 few-shot 未内联」原为刻意取舍（提示词调优产物 + 33 kB 副本漂移风险，见插件 provenance
注释）。批次 7 激活后提示词直接承载生产抽取质量，该取舍被推翻：上游 `# EXAMPLES` 区块
（14 kB，11 个示例，上游编号 1-12 跳过 4——矩阵此前的「12 个」按最大编号记录，实际 11 块）
以 `ADDITIVE_EXTRACTION_EXAMPLES` 常量逐字内联，来源 `47a69e1`；锚定测试钉住尺寸、标记、
全部示例头与渲染包含性，截断与漂移不再可能静默发生。组装顺序有一处已记录偏差：示例块位于
契约段（含输出格式）之后而非上游的之前——避免拆分既有契约常量，二者均属提示词调优自由度。
**部分 7 项中 4 项关闭**（#2 threshold/explain、#3 over-fetch、#6 role、#7 few-shot），
剩余 3 项（#1 add 参数面的 infer 分量已在批次 17 激活、#4 update 重链接、#5 时间锚点）
均为运行期增强项。**至此，mem0 提示词资产的全部契约与调优面在本仓齐备。**


**批次 19 补记（2026-09-25，观察日期参数化 + 两项终态处置）—— 部分项清零**：

1. **时间锚点（部分 #5）关闭**：`MemoryExtractionRequest` 新增 `observationDate`
   （`YYYY-MM-DD`，入口轻校验），经抽取提示词的 `Observation Date` 段直达模型——
   mem0 用于解析相对时间引用的唯一时间锚点自此可由调用方控制（缺省今日，与上游实际行为一致）。
   测试经捕获的模型提示词断言锚点与 custom instructions 真实进入提示词。
2. **update 实体重链接（部分 #4）终态处置：架构不适用**。mem0 需要在文本更新时重建实体链接，
   因为其链接**从文本派生**；本仓的 provenance 边是**作者式**的（商业面显式创建，`source_memory_id`
   记录佐证关系）——记忆文本更新不改变「这条边由该记忆佐证」的作者事实，链接不失效。派生式
   链接若未来引入，其重链逻辑归属抽取管线演进，不属于记录层缺口。
3. **向量 retriever 单槽取舍（批次 7 收尾）终态处置：已被取代**。批次 8b 之后，向量语义分由
   服务层直供（`compute_vector_similarities` → 8a 接收端），排序职责不再依赖插件向量检索器；
   插件的向量索引保留为纯向量部署的可选替代实现。单槽冲突从「行为阻断」降级为「部署拓扑选项」。

**对齐终态（2026-09-25，批次 19 后）**：

| 判定 | 数量 | 说明 |
| --- | --- | --- |
| 缺失 | **0** | 批次 14 清零后保持 |
| 部分 | **0** | 5 项关闭 + 2 项终态处置（本批次） |
| 偏离（已记录） | 4 | BM25 对齐 ES 分支、PROPER/TOPIC 确定性近似、few-shot 组装顺序、示例块位置（均有书面理由与测试/锚定） |
| 对齐 + 超越 | 45+ | 全量 |
| 待运行数据决策 | 2 | 加法归一策略的默认值取舍；few-shot 与提示词资源的线上效果调优——两者均为运营优化，非能力缺口 |**行为对齐的必要面至此全部就位。**
